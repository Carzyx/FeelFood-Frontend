...
export class RateComponent implements OnInit, ControlValueAccessor, Validator {
    ...
    ngOnInit() {
        this.buildRanges();
        this.writeValue(this.ratedValue);
        this.getIcon();
    }
    ...
    private getIcon(){
        var icon = this.iconCollection[this.iconClass]

        if(!icon)
        {
            //set default icon
            this.iconClass = "star";
            this.titleMessage = "Star";            
            return;
        }
        
        this.titleMessage = this.iconClass;
        this.iconClass = this.iconClass.toLocaleLowerCase();
        this.fullIcon = icon[0];
        this.emptyIcon = icon[1];
    }
}